...
module.exports = {
	...
	output: {
		...
		library: '_dll_[name]_[fullhash]' // 全局变量名称 导出库将被以var的形式赋给这个全局变量 通过这个变量获取到里面模块
	},
	plugins: [
		new webpack.DllPlugin({
			// path指定manifest文件的输出路径
			path: path.join(__dirname, '../dll', '[name].manifest.json'),
			context: __dirname,
			name: '_dll_[name]_[fullhash]' // 和library 一致，输出的manifest.json中的name值
		})
	]
};
